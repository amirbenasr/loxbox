...
class LoxboxTaskModuleFrontController extends ModuleFrontControllerCore
{
    ...
    public function initContent()
    {
        ...
        if (Tools::getValue('ajax') && Tools::getValue("product_id")) {
            ...
            $sql = 'SELECT COUNT(*) FROM `' . _DB_PREFIX_ . '`address WHERE id_customer=`' . $user_id . '` and other=`' . $name . '` and id_address=(SELECT max(id_address) FROM `' . _DB_PREFIX_ . '`address);';
            $count = $db->getValue($sql);

            if ($count != 0) {

                $sql = 'SELECT MAX(id_address) from `' . _DB_PREFIX_ . '`address where id_customer=`' . $user_id . '`';
